...
export async function GET() {
  ...
  try {
    ...
    const formattedTrends = monthlyTrends.rows.map((row: any) => {
      return {
        month: row.month,
    ...
    const latestMonth = formattedTrends[formattedTrends.length - 1]
    const previousMonth = formattedTrends[formattedTrends.length - 2]
    const yearAgo = formattedTrends[Math.max(0, formattedTrends.length - 13)]

    const momChange = previousMonth
      ? ((latestMonth.median_price - previousMonth.median_price) / previousMonth.median_price) * 100
      : 0

    const yoyChange = yearAgo
      ? ((latestMonth.median_price - yearAgo.median_price) / yearAgo.median_price) * 100
      : 0

    // Executive summary metrics
    const executiveSummary = {
      current_median_price: latestMonth.median_price,
      ...
      data_coverage_months: formattedTrends.length,
    }
